function net = alexnet_init(varargin)
...
switch opts.model
  case 'alexnet'
    net.meta.normalization.imageSize = [227, 227, 6] ;
%     net = load('/media/vclagpu/Data12/zhou/My_Items/interpretableCNN/nets/imagenet-caffe-alex.mat'    
    net = alexnet(net, opts);
%     net.layers = net.layers(1:19);
%     net.layers{end+1} = struct('type', 'mask', 'name', 'mask') ;
%     net = add_block(net, opts, '5', 1, 2, 384, 4096, 1, 0) ;
%     net = add_block(net, opts, '6', 1, 1,4096, 1000, 1, 0) ;
%     net = add_block(net, opts, '9', 1, 1,1000, 1, 1, 0) ;
%     net.layers{end+1} = struct('type', 'sigmoid', 'name', 'final_sigmoid');
...
net.layers{end+1} = struct('type', 'loss', 'name', 'logitstic') ;
...
net = add_block(net, opts, '1', 11, 11, 6, 96, 4, 0) ;
