function res=my_alexnet(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'mask'
      si = size(res(i).x);
      res(i+1).x = gpuArray(zeros(1,2,si(3),si(4),'single'));
      for k=1:si(4)
          res(i+1).x(1,1,:,k) = res(i).x(l.masks(1,k),l.masks(2,k),:,k);
          res(i+1).x(1,2,:,k) = res(i).x(l.masks(3,k),l.masks(4,k),:,k);
      end
    ...
    case 'myloss'
      res(i+1).x = myloss(res(i).x, l.class, 'mse');
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'mask'
        si=size(res(i).x);
        res(i).dzdx = gpuArray(zeros(si,'single'));
        for k=1:si(4)
          res(i).dzdx(l.masks(1,k),l.masks(2,k),:,k) = res(i+1).dzdx(1,1,:,k);
          res(i).dzdx(l.masks(3,k),l.masks(4,k),:,k) = res(i+1).dzdx(1,2,:,k);
        end
      case 'convt'
        [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
      ...
      case 'myloss'
        res(i).dzdx = myloss(res(i).x, l.class, 'mse', res(i+1).dzdx);
      case {'ourloss_logistic','ourloss'}
