function [net, stats] = alexnet_train(net, imdb, getBatch, varargin)
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'abs_err'
      opts.errorFunction = @error_abs ;
      if isempty(opts.errorLabels), opts.errorLabels = {'absoluteError'} ; end
    otherwise
...
% -------------------------------------------------------------------------
function err = error_abs(params, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-1).x);
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    res=my_alexnet(net, im, dzdy, res, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
