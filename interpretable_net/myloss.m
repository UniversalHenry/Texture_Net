function [y,y_] = myloss(x,c,theType,varargin)
...
while ~isempty(varargin)
    switch lower(varargin{1})
        case 'peaks'
            opts.peaks = varargin{2};
        case 'tau'
            opts.tau = varargin{2};
        case 'peak_coord'
            opts.peak_coord = varargin{2};
        case 'strength'
            opts.strength = varargin{2};
        case 'weight'
            opts.weight = varargin{2};
    end
    varargin(1:2)=[];
end
...
if ~strcmp(theType,'texture_loss')
...
switch lower(opts.loss)
  ...
  case {'binaryerror', 'binarylog', 'logistic', 'hinge', 'mse', 'texture_loss'}
...
if nargin <= 2 || isempty(dzdy)
  switch lower(opts.loss)
    ...
    case 'texture_loss'
      peak1 = opts.peaks(:,1,:,:);
      tau = opts.tau;
      l1_pos = (tau>peak1).*(tau-peak1).*(c>=0);
      l1_neg = (tau<peak1).*(tau-peak1).*((c<0) .* (c>=-1))*(-1);
      t1 = l1_pos;
      t2 = l1_neg;
  end
  if ~isempty(instanceWeights)
    y = instanceWeights(:)' * t1(:) ;
    y_ = instanceWeights(:)' * t2(:) ;
  else
    if sum(reshape(c>=0,1,[]))>0
        y = sum(t1(:));%./sum(reshape((c>=0),1,[]));
    else
        y = 0;
    end
    if sum(reshape((c>=-1).*(c<0),1,[]))>0
        y_ = sum(t2(:));%./sum(reshape((c>=-1).*(c<0),1,[]));
    else
        y_ = 0;
    end
  end
  ...
  switch lower(opts.loss)
    ...
    case 'texture_loss'
      peak1 = opts.peaks(:,1,:,:);
      peak1_coord = opts.peak_coord(1:2,:,:);
      tau = opts.tau; 
      y = gpuArray(zeros(size(x),'single'));
      p1_grad = -c.*((tau-peak1).*c>0).*(c>=-1); %[1,1,channel,batch]
      for i=1:size(x,4)
          for j=1:size(x,3)
              y(peak1_coord(1,j,i), peak1_coord(2,j,i), j, i) = p1_grad(1,1,j,i);
          end
      end
  end
end
