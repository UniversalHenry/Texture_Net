function [net, stats] = my_vgg_train(net, texture_net, imdb, getBatch, varargin)
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'fmap'
      opts.errorFunction = @error_fmap ;
      if isempty(opts.errorLabels), opts.errorLabels = {'lossPositive', 'lossNegative'} ; end
    otherwise
...
for epoch=start+1:opts.numEpochs
  ...
  params = opts ;
  params.epoch = epoch ;
  params.saveDir = fullfile(opts.expDir, sprintf('epoch_%d',epoch));
  params.learningRate = opts.learningRate(min(epoch, numel(opts.learningRate))) ;
  % shuffle
  if mod(numel(opts.train),2)==0
      tmp_shuffle=randperm(numel(opts.train)/2);
      params.train = opts.train(reshape([tmp_shuffle*2-1;tmp_shuffle*2],1,[])) ; 
      params.train = params.train(1:min(opts.epochSize, numel(opts.train)));
  else
      tmp_shuffle=randperm(floor(numel(opts.train)/2));
      params.train = opts.train([reshape([tmp_shuffle*2-1;tmp_shuffle*2],1,[]),numel(opts.train)]) ; 
      params.train = params.train(1:min(opts.epochSize, numel(opts.train)));
  end
  if mod(numel(opts.val),2)==0
      tmp_shuffle=randperm(numel(opts.val)/2);
      params.val = opts.val(reshape([tmp_shuffle*2-1;tmp_shuffle*2],1,[])) ; 
      params.val = params.val(1:min(opts.epochSize, numel(opts.val)));
  else
      tmp_shuffle=randperm(floor(numel(opts.val)/2));
      params.val = opts.val([reshape([tmp_shuffle*2-1;tmp_shuffle*2],1,[]),numel(opts.val)]) ; 
      params.val = params.val(1:min(opts.epochSize, numel(opts.val)));
  end
  params.imdb = imdb ;
  ...
  if numel(params.gpus) <= 1
    [net, state] = processEpoch(net, texture_net, state, params, 'train',imdb.meta.dataMean) ;
    [net, state] = processEpoch(net, texture_net, state, params, 'val',imdb.meta.dataMean) ;
    if ~evaluateMode
    ...
    spmd
      [net, state] = processEpoch(net, texture_net, state, params, 'train') ;
      [net, state] = processEpoch(net, texture_net, state, params, 'val') ;
...
err = sum(error(:)) ;
% -------------------------------------------------------------------------
function err = my_error_binary(params, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-3).x) ;
error = bsxfun(@times, predictions, labels) < 0 ;
err = sum(error(:)) ;
% -------------------------------------------------------------------------
function err = error_fmap(params, labels, res)
% -------------------------------------------------------------------------

err(1,1) = sum(double(gather(res(end).pos_l))) ;
err(2,1) = sum(double(gather(res(end).neg_l))) ;
...
% -------------------------------------------------------------------------
function [net, state] = processEpoch(net, texture_net, state, params, mode,imdbMean)
% -------------------------------------------------------------------------
...
subset = params.(mode) ;
num = [0;0;0;0;0] ; %for loss_negative, loss_positive, loss_total, binerr, classificationloss
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    [vgg_im, labels] = params.getBatch(params.imdb, batch) ;
    ...
    net.layers{end-2}.precious=1;
    net.layers{end-2}.density=density;
    net.layers{end-10}.precious=1;
    net.layers{end-3}.precious=1;
    net.layers{end}.precious=1;
    res = my_vgg(net, texture_net, vgg_im, dzdy, res,imdbMean, ...
                      params.epoch, params.saveDir, fix((t-1)/params.batchSize)+1,...
                      params.numEpochs,...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
    ...
    error = sum([error, [...
      ...
      reshape(params.errorFunction(params, [], res),[],1) ;
      sum(double(gather(res(end-2).x)));
      my_error_binary(params, net.layers{end-2}.class, res)]],2) ;
  end
  ...
  %update num
  pos_num = sum((res(end).class(:)>=0).*(net.layers{end}.tau>res(end).peaks(:)));
  neg_num = sum(((res(end).class(:)<0).*(res(end).class(:)>=-1)).*(net.layers{end}.tau<res(end).peaks(:)));
  num = num + [pos_num+neg_num; pos_num; neg_num;numel(batch);numel(batch)];
  
  % get statistics
  time = toc(start) + adjustTime ;
  batchTime = time - stats.time ;
  errortmp = error ./ gather(num);
  stats = extractStats(net, params, gather(errortmp)) ;
  stats.binloss = gather(errortmp(4));
  stats.binerr = gather(errortmp(5));
  stats.num = gather(num) ;
  stats.time = time ;
...
%   score = cat(2,score,reshape(res(end).class,[96,batchSize]));
%   peaks = cat(2,peaks,reshape(res(end).peaks,[96,batchSize]));
  ...
  fprintf('\n') ;
%   if (fix((t-1)/params.batchSize)+1 == ceil(numel(subset)/params.batchSize)) && strcmp(mode, 'train')
%       if ~exist(params.saveDir)
%           mkdir(params.saveDir);
%       end
%       save([params.saveDir,'/score.mat'],'score');
%       save([params.saveDir,'/peaks.mat'],'peaks');
%       for k=1:96
%           f=figure('visible', 'off');
%           hist(score(k,:), 30);
%           savefig(f,[params.saveDir,sprintf('/score_dist_filter_%d.fig',k)]);
%           imwrite(f,[params.saveDir,sprintf('/score_dist_filter_%d.fig',k)]);
%           f2=figure('visible', 'off');
%           hist(peaks(k,:), 30);
%           %savefig(f2,[params.saveDir,sprintf('/peak_dist_filter_%d.fig',k)]);
%           imwrite(f2,[params.saveDir,sprintf('/peak_dist_filter_%d.fig',k)]);
%       end
%   end
