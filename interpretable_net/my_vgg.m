function res=my_vgg(net, texture_net, x, dzdy, res,imdbMean, epoch, saveDir, ...
    batch_num, numEpochs, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'texture_net'
      [labels, peaks, peak_coord] = pass_to_texturenet(res(i-8).x,res(i-1).x,texture_net,x,l.side,opts);
      net.layers{end}.peaks = peaks;
      net.layers{end}.peak_coord = peak_coord;
      net.layers{end}.class = labels;
      res(i+1).x = res(i-8).x;
      
      %%save some samples
      if opts.epoch >= opts.numEpochs
           save_img( opts.saveDir, x, res(i-8).x,res(i-1).x, labels,...
                peak_coord, net.layers{end}.tau, opts.batch_num, l.side,imdbMean)
      end
    ...
    case 'loss'
      res(i+1).x = vl_nnloss(res(i).x, l.class, 'loss', 'logistic') ;
      
    case 'myloss'
      [pos_l, neg_l] = myloss(res(i).x, l.class, 'texture_loss',...
                            'peaks',l.peaks,...
                            'tau', l.tau);
      res(i+1).x = pos_l + neg_l;
      res(i+1).pos_l = pos_l;
      res(i+1).neg_l = neg_l;
      res(i+1).class = l.class;
      res(i+1).peaks = l.peaks;
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'loss'
        res(i).dzdx = vl_nnloss(res(i).x, l.class, res(i+1).dzdx, 'loss', 'logistic') ;
      case 'myloss'
        res(i).dzdx = myloss(res(i-9).x, l.class, 'texture_loss', res(i+1).dzdx,...
                            'peaks',l.peaks,...
                            'tau', l.tau,...
                            'peak_coord', l.peak_coord);
      ...
      case 'relu'
          switch l.name
              case 'relu_texture'
                res(i+1).dzdx = res(i+1).dzdx + res(i+10).dzdx;
                if l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
                if ~isempty(res(i).x)
