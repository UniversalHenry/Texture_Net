...
num_neg=floor(length(objset_neg)/2)*2;
tarN=floor(posRate/(1-posRate)*num_neg/2)*2;
repN=ceil(tarN/(num_pos*2));
list_train_pos=round(linspace(1,num_pos,round(num_pos*trainRate/2)*2));
...
list_train_neg=round(linspace(1,num_neg,round(num_neg*trainRate/2)*2));
...
if(IsTrain)
    rng(0);
    list_tmp1=[randperm(tarN),randperm(num_neg)+tarN];
%     list_tmp2=randperm(total_images/2);
%     list=list_tmp1(reshape([list_tmp2*2-1;list_tmp2*2],1,[]));
    list=list_tmp1;
