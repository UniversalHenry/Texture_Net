...
%       peak2 = opts.peaks(:,2,:,:);
      ...
      l1_neg = (tau<peak1).*(tau-peak1).*(c<0)*(-1);
%       l2 = (tau>peak2).*(tau-peak2).*c;
%       t = (l1 + l2)/size(c,3)/size(c,4);
      t1 = l1_pos;
      t2 = l1_neg;
...
%           t = load('diff.mat');
%           data = t.data;
%           data = cat(2, data, squeeze(l1_pos));
%           save('diff.mat', 'data');
%       end
  end
  if ~isempty(instanceWeights)
  ...
  switch lower(opts.loss)
    ...
    case 'texture_loss'
      ...
      p1_grad = -c.*((tau-peak1).*c>0); %[1,1,channel,batch]
      for i=1:size(x,4)
          for j=1:size(x,3)
